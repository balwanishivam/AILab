...
public class q1_Graph
{
			...
			for (int i = 0;i < nodes;i++)
			{
				for (int j = 0;j < nodes;j++)
				{
					if (a[i][j] == -1)
					{
						...
						{
							a[i][j] = (long) new Random().nextInt(60) + 1;
						}
							a[j][i] = a[i][j];
					}
				}
		...
		public final ArrayList<Long> generateChild(ArrayList<Long> t)
		{
			ArrayList<Long> temp = new ArrayList<Long>();
				...
				do
				{
					one = (int) new Random().nextInt((int)nodes);
					two = (int) new Random().nextInt((int)nodes);
